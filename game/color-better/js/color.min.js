...
(function() {
	...
	var e = s.each = s.forEach = function(G, J, H) {
		...
		if (M && G.forEach === M) {
			...
			if (G.length === +G.length) {
				...
				for (var C = s.keys(G), L = 0, K = C.length; K > L; L++) {
	...
	s.groupBy = o(function(H, G, C) {
		...
		K = k(K);
		for (var Q = K.call(J, L), P = 0, C = G.length; C > P;) {
			var H = P + C >>> 1;
	...
	s.flatten = function(C, G) {
		...
		for (var H = s.max(s.pluck(arguments, "length").concat(0)), G = new Array(H), C = 0; H > C; C++) {
	...
	s.bind = function(H, G) {
		...
		for (var C = {}, K = s.keys(H), J = 0, G = K.length; G > J; J++) {
			C[H[K[J]]] = K[J];
		}
		return C;
		...
		var G = {},
			C = i.apply(l, A.call(arguments, 1));
		return e(C, function(J) {
			J in H && (G[J] = H[J]);
		}), G;
	}, s.omit = function(H) {
		var C = {},
			J = i.apply(l, A.call(arguments, 1));
		for (var G in H) {
	...
	s.template = function(P, C, U) {
		...
		var G = new RegExp([(U.escape || E).source, (U.interpolate || E).source, (U.evaluate || E).source].join("|") + "|$", "g"),
			L = 0,
			H = "__p+='";
...
! function() {
	var b = $("#box"),
		...
		a = {
			...
			renderUI: function() {
				var d = 90 == window.orientation || -90 == window.orientation,
					e = d ? window.innerHeight : window.innerWidth;
			...
			gameOver: function() {
				...
				this.lastLv = this.lv, this.lastGameTxt = e.txt, this.lastGamePercent = e.percent, c.d_share.hide().find("h3").text(this.lastGameTxt), c.d_share.hide().find("h2").text(this.lv + "关"), c.d_content.hide(), c.d_gameover.show().find("h3").text(this.lastGameTxt), c.d_gameover.show().find("h2").text(this.lv + "关"), b.find("span").fadeOut(1000, function() {
					c.dialog.fadeIn();
				}), this._pause = !0, this.reset();
				setTimeout(function() {
					(adsbygoogle = window.adsbygoogle || []).push({});
				}, 1000);
				var f = Game.lastLv > 0 ? "我闯过" + (Game.lastLv) + "关，我的视力超过" + Game.lastGameTxt + "！不服來战！" : "保护眼睛，16关以下不要再玩手机了！"; //DouyaGame.resetShareText(f);
			},
			reset: function() {
...
function(b) {
	var c = {
		...
		a = {
			...
			initEvent: function() {
				var d = "ontouchstart" in document.documentElement ? "touchstart" : "click",
					e = this;
...
function() {
	var d = $("#box"),
		...
		b = $("#help_color"),
		c = {
			lvT: ["基本上是瞎子！", "相当于鼴鼠的视力！", "低于大部分人的水平", "接近正常人的水平", "达到了正常人的水平", "超过大部分人的水平", "优秀级水平", "可以去“最强大脑”了", "对色差具有超凡的能力！作弊？？"],
			render: function(h, i) {
				...
				var m = Math.floor(Math.random() * h * h),
					f = this.getColor(255 - this.d),
					g = this.getLvColor(f[0]);
				d.find(e).css("background-color", f[1]).data("type", "b"), d.find(e).eq(m).css("background-color", g[1]).data("type", "a");
			},
			getColor: function(g) {
				var h = [Math.round(Math.random() * g), Math.round(Math.random() * g), Math.round(Math.random() * g)],
					f = "rgb(" + h.join(",") + ")";
				return [h, f];
			},
			getLvColor: function(h) {
				var i = this.d,
					f = _.map(h, function(j) {
						return j + i;
					}),
					g = "rgb(" + f.join(",") + ")";
				return [f, g];
			...
			getGameOverText: function(g) {
				var h = 15 > g ? 0 : Math.ceil((g - 15) / 5),
					i = this.lvT[h] || _.last(this.lvT),
					j = i,
					f = 2 * g * 0.95;
				return f = f > 90 ? 90 + 0.15 * g : f, f = Math.min(f, 100), {
					txt: j,
					percent: f.toFixed(2)
				};
			}
